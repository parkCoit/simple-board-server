...
class KakaoRepository(object):
    ...
    def get_jwt(self, user_info):
        ...
        encoded_jwt = jwt.encode({'id': user_info['id'], 'nickname': user_info['nickname'], 'datetime': dt},
                                 SOCIAL_OUTH_CONFIG['KAKAO_SECRET_KEY'], algorithm=SOCIAL_OUTH_CONFIG['ALGORITHM'])
        kakao_token = {"token": encoded_jwt, 'id' : user_info['id']}
        payload = jwt.decode(encoded_jwt, SOCIAL_OUTH_CONFIG['KAKAO_SECRET_KEY'],
                             algorithms=SOCIAL_OUTH_CONFIG['ALGORITHM'])
        print(payload)
        return kakao_token
